...
To evaluate LLM multi-channel orchestration of a fixed tool surface.
Four parallel decision channels operate at three nested cadences
over a fixed 4-tool layer with schema-validated directives.
Existing benchmarks evaluate single agents picking single tools
(AgentBench, GAIA, ToolLLM) or flat MARL on stateless games
(MeltingPot, Crafter, BALROG); the orchestration-of-fixed-tools
...
Evaluating (i) multi-channel orchestration of a fixed tool surface;
(ii) schema-validity profiling under multi-hour coherence stress;
(iii) reproducibility-aware LLM evaluation methodology
(LayerCast, RecordReplayCache); (iv) sample-efficient learning
under the 4-channel directive surface as the action space (future
