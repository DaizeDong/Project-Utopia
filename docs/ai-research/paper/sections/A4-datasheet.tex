...
To quantify stale coherence --- the failure mode where an LLM in a
long-horizon agent setting continues to \emph{verbally} mention an
anchor while its \emph{action-emitting} directives stop reflecting
it. Existing memory benchmarks (LongMemEval, RULER,
Lost-in-the-Middle, InfiniteBench, MemoryArena, MemoryAgentBench)
measure only one axis (verbal recall) or operate in session-discrete
loops; none jointly measure verbal-recall $\times$
action-grounded-recall on tick-continuous naturally-accumulated
context.
...
Self-funded. All LLM API costs for the 3-experiment matrix
($\sim$95 runs: 40 for E1, 25 for E2, 30 for E6) are paid out of
pocket. No grant identifiers.
...
Unbounded by construction: 6 scenarios (4 used in the paper-run
matrix) $\times$ $2^{32}$ seeds $\times$ arbitrary tick budgets.
The paper-run matrix materializes $\sim$95 trajectories
(3 experiments $\times$ 4 scenarios $\times$ 5--10 seeds) on
demand from the released CLI.
\item \textbf{What does the released artifact contain?}
(1) Four verbatim system prompts in \texttt{project\_utopia/data/prompts/}
defining the LLM contract surface --- one per decision channel
(\emph{environment-director}, \emph{npc-policy},
\emph{strategic-plan}, \emph{colony-agent});
(2) six hand-tuned scenario blueprints (\S\ref{app:scenarios});
(3) the deterministic simulator core
($\sim$23\,k LOC Python, 128 modules);
...
The 3 experiments (E1, E2, E6) define the evaluation cells;
within each, seeds 0--9 are the canonical splits. Held-out seeds
10--19 are reserved for replication and not used in any reported
finding.
...
The 3-experiment paper-run matrix in this submission
(\S\ref{sec:experiments}). No external uses to date.
\item \textbf{What other tasks could this be used for?}
(i) measuring stale coherence across LLM families on long-horizon
structured-output tasks; (ii) ablating the contribution of
context-window size vs anchor design vs model scale to stale
coherence; (iii) reproducibility-aware LLM evaluation methodology
(LayerCast, RecordReplayCache); (iv) extending the dual-probe
(verbal $\times$ action-grounded recall) methodology to other
domains.
...
Project-Utopia is not a benchmark of language capability in
isolation. Stale-coherence scores do not directly predict
GPQA / MMLU / HumanEval. Conversely, GPQA-strong models may
fail Project-Utopia for stale-coherence reasons orthogonal to
reasoning. The benchmark is also \emph{not} a substitute for:
...
verbal--action dissociation under a long-horizon
structured-output substrate, nothing broader
