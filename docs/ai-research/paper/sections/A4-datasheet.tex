...
entity?}
Daize Dong, as an independent academic submission. No corporate or
institutional sponsorship. Affiliation pending.
...
(1) Four verbatim system prompts in \texttt{src/data/prompts/}
...
($\sim$55\,k LOC Node.js, $\sim$5\,500 LOC Python);
...
\texttt{Guardrails.js} / \texttt{guardrails.py} before being
forwarded to the substrate (weights $\in [0,3]$, risk $\in
[0,1]$, TTL $\in$ [8\,h, 24\,h]; full table in
...
Yes: pydantic schema, JSON-Schema definition
(\texttt{src/simulation/ai/llm/ResponseSchema.js}),
\texttt{Guardrails.js}, and the 5-dimension plugin code are all
in the release tree under Apache 2.0 / MIT.
\end{itemize}
...
Yes, publicly. The Node.js implementation is on GitHub at
\texttt{[ANONYMIZED URL]} (de-anonymized to
\texttt{https://github.com/DaizeDong/Project-Utopia} post-review).
The Python SDK is on PyPI as \texttt{project-utopia}.
\item \textbf{How will it be distributed?}
Git tag \texttt{refactor/academic-benchmark-py-rc1} for the
Node.js core; \texttt{pip install project-utopia} for the
Python SDK. A Docker image pinned to commit \texttt{fe55b82}
...
The author, via the GitHub repository above. The Python
SDK is auto-published from the same source tree on tag.
