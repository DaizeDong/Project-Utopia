# Changelog — Pre-Python Era (v0.7 – v0.10)

Historical changelog entries from the JavaScript-era of Project Utopia (v0.7.x – v0.10.x).
These entries pre-date the Python migration (early 2026) and the academic-benchmark refactor.
See the current `CHANGELOG.md` at the repo root for Python-era / academic-refactor entries.
...
Implements the P1 user-directive ecology fix from `assignments/homework7/Final-Polish-Loop/Round13/Plans/Plan-R13-wildlife-hunt.md` (R13 user issue #8). User report: "(a) Increase wildlife spawn frequency. (b) Add 1-2 more wildlife species variety (use existing animal types — DON'T add new role/mechanic; just spawn more variety from existing pool). (c) Worker hunts predator/wildlife → add reward (food drop on wildlife death; recordResourceFlow food/produced; existing carry already has food)." Pre-fix `WildlifePopulationSystem` populated `predatorsBySpecies = { wolf, bear, raider_beast }` but `EntityFactory.pickPredatorSpecies` weighted them 55/30/15, so wolf dominated visible spawns. Recovery cooldowns (75 s herbivore / 120 s predator) were too long to feel an active ecosystem, and a worker downing a predator yielded zero economic upside, so workers never opportunistically hunted.

**Files changed:** 3 source modified — `src/config/balance.js` (+10 LOC: 3 new constants `wildlifeSpawnIntervalMult=0.5`, `wildlifeSpeciesRoundRobin=true`, `wildlifeHuntFoodReward=4` + commentary citing this plan), `src/simulation/ecology/WildlifePopulationSystem.js` (+~30 LOC: `PREDATOR_SPECIES_POOL` + `pickRoundRobinPredatorSpecies` helper that reads `state.metrics.ecology.predatorsBySpecies` and returns the least-represented species; `spawnAnimals` passes the forced species through to `createAnimal`'s 5th arg when round-robin is enabled and `kind===PREDATOR`; the 3 spawn-cooldown sites in `updateZonePopulation` (`nextRecoveryAtSec`, `nextBreedAtSec`, `nextPredatorRecoveryAtSec`) all multiply the tuning cooldown by `intervalMult`), `src/simulation/npc/fsm/WorkerStates.js` (+~22 LOC: import `recordResourceFlow`; the existing kill block in FIGHTING.tick now drops `wildlifeHuntFoodReward` food into the killer's carry up to `workerDeliverThreshold × 2` cap, overflows into `state.resources.food`, and records a `food/produced` flow). 1 new test — `test/wildlife-hunt-reward.test.js` (~110 LOC, 3 cases: spawn cooldown halved by mult, round-robin picks least-represented species, worker kill drops food + records flow). Total: ~62 production LOC + 110 test LOC = 172 LOC. Within plan's ~60 production LOC scope.

**Acceptance:** Spawn cadence visibly faster — `nextRecoveryAtSec` advances by `cooldown × 0.5` so the 75s herbivore recovery window collapses to 37.5s, and the 120s predator recovery to 60s. Round-robin sort over `predatorsBySpecies` ensures the species with the smallest live count is picked first, so `bear` and `raider_beast` actually appear within minutes of play instead of being weighted-randomly starved by wolf's 55% bias. Worker hunt deposits 4 food per kill — into carry until `workerDeliverThreshold × 2 = 5` cap, then overflow into colony stockpile, with `recordResourceFlow(state, "food", "produced", 4)` so the production-rate HUD reflects the hunt loop. The kill block only fires for `target.type === "ANIMAL" && target.kind === "PREDATOR"` so saboteur kills (which already use the same code path) are NOT food-rewarded. Hard-freeze compliant: zero new species, zero new role, zero new mechanic; just enables existing pool members to spawn more often / be picked round-robin / drop food. Test baseline preserved — full suite **2033 pass / 0 fail / 4 skip**.

## [Unreleased] — v0.10.1-p (R13 Plan-R13-autopilot-wait-llm, P1)

### Plan-R13-autopilot-wait-llm — Autopilot waits for first /api/ai/plan response (or fallback / 10s timeout) before placing buildings

Implements the P1 user-directive startup-gate fix from `assignments/homework7/Final-Polish-Loop/Round13/Plans/Plan-R13-autopilot-wait-llm.md` (R13 user issue #6). User report: "Autopilot currently builds immediately. Should wait for first LLM /api/ai/plan response before issuing build proposals (unless fallback mode active). Add state.ai.autopilotReady = false until first plan received OR state.ai.fallbackMode = true." Pre-fix `BuildAdvisor` (via `ColonyDirectorSystem`) ran every tick from sim start — the first 1.5–3 sec window placed 3–6 buildings via local heuristics + proposer chain before the LLM response landed, anchoring the colony layout where the LLM could only add to it, never undo. Per the directive, autopilot now holds off building until either (a) the first `/api/ai/plan` response is received OR (b) fallback mode is explicitly active OR (c) a 10-sec safety timeout fires.

**Files changed:** 4 source modified — `src/entities/EntityFactory.js` (+10 LOC: 4 new `state.ai` fields `autopilotReady=false`, `firstPlanReceivedSec=null`, `autopilotReadyReason=null`, `fallbackMode=false` with commentary citing this plan), `src/app/aiRuntimeStats.js` (+22 LOC inside `recordAiDecisionResult`: first non-fallback response flips `autopilotReady=true / autopilotReadyReason="first-plan" / firstPlanReceivedSec=nowSec`; first fallback response flips `autopilotReady=true / fallbackMode=true / autopilotReadyReason="fallback"`), `src/config/balance.js` (+7 LOC: new constant `autopilotReadyTimeoutSec=10` next to `policyDecisionIntervalSec`), `src/simulation/meta/ColonyDirectorSystem.js` (+30 LOC: import `BALANCE`, gate `update()` on `state.ai.autopilotReady === false` — early-return with `automation.phaseBuilder = "awaiting-first-plan"`; the safety timeout flips `fallbackMode=true / autopilotReady=true / autopilotReadyReason="timeout"` when `nowSec >= timeoutSec`). 1 new test — `test/autopilot-wait-llm.test.js` (+95 LOC, 7 cases: initial-state defaults, gated early-return, first-plan flip, fallback flip, post-flip director runs, safety timeout fires, manual mode never gated). Total: ~69 production LOC + 95 test LOC = 164 LOC. Within the plan's ~40 production LOC scope estimate (the +30 LOC in ColonyDirectorSystem includes the optional safety-timeout branch + automation-block update which the plan called out as Step 3).

**Back-compat:** the gate condition uses strict `=== false` rather than `!== true`. Tests / older saves that overwrote `state.ai` without the new field (where `autopilotReady === undefined`) are treated as already-ready, matching the legacy immediate-build behaviour they were written against. `recordAiDecisionResult` uses `autopilotReady !== true` so it'll set the flag on first response regardless of starting value.

**Layering with Plan-R13-fog-aware-build (54cb911):** that plan's per-candidate fog gate lives INSIDE `findPlacementTile`. This plan's startup-readiness gate lives ABOVE the entire build loop in `ColonyDirectorSystem.update`. Composition: when autopilot is ON and not-yet-ready, we early-return before the fog gate ever runs. Once readiness clears (LLM plan / fallback / timeout), the fog gate's per-candidate skip + `state.ai.scoutNeeded` latch behaves exactly as before. Layered cleanly per the plan's stated merge order.

**Acceptance:** With `state.ai.enabled=true` and `state.ai.autopilotReady=false`, the director's phase-builder + scenario repair both hold (`automation.phaseBuilder === "awaiting-first-plan"`, `automation.scenarioRepairAllowed === false`, `buildsPlaced === 0`). First valid LLM `/api/ai/plan` response (any non-fallback `recordAiDecisionResult` call) flips `autopilotReady=true / autopilotReadyReason="first-plan"`; the next director tick runs normally. First fallback response also flips the gate (`fallbackMode=true / autopilotReadyReason="fallback"`) so an offline / no-key run isn't stalled. After `BALANCE.autopilotReadyTimeoutSec=10` sim-sec elapses without any response, the gate self-clears with `autopilotReadyReason="timeout"`. Manual mode (`state.ai.enabled=false`) is never gated. Test baseline preserved — same single pre-existing fail (`exploit-regression: exploit-degradation`, latent since v0.8.7) as parent commit `17af3cb`; full suite 2030 pass / 1 fail / 3 skip.
...
Implements the P1 user-directive UX fix from `assignments/homework7/Final-Polish-Loop/Round13/Plans/Plan-R13-chip-label.md` (R13 user issue #3). User report: "Top scenario goal chips show '3/8' but no building name. Add label like 'Farms 3/8' + ensure no overlap with adjacent chips." Pre-fix the chip name span DID emit `${chip.name} ` (lowercase, e.g. "farms 3/8") but the user-perceived "no building name" stemmed from (a) the lowercase form looking like a fragment rather than a label, and (b) the ≤1280 px responsive band hiding the name span entirely (icon-only mode at narrow viewports). Suggestion A from the plan: capitalize the displayed name span ("Farms" / "Warehouses" / "Herb Garden") so the label reads as a proper word, while keeping the underlying `chip.name` field lowercase so legacy plain-text consumers (chip.label, dev-mode scenario string, hover tooltip parsers) continue to read "farms 3/8" without modification.

**Files changed:** 1 source modified — `src/ui/hud/HUDController.js` (+~17 LOC: new `capitalizeChipName` helper that handles single words ("farms" → "Farms") and snake_case multi-word names ("herb_garden" → "Herb Garden"); the helper is invoked at the chip-name span textContent assignment in `#renderGoalChips`, replacing the literal `${chip.name} ` template with `${capitalizeChipName(chip.name)} `). 1 existing test relaxed — `test/hud-goal-chips.test.js` (2 lines: changed `/warehouses \d+\/\d+/` and `/farms \d+\/\d+/` to case-insensitive `/i` flag so both the legacy lowercase and the new capitalized form pass). 1 new test — `test/hud-goal-chip-label.test.js` (~190 LOC including the shared in-process DOM mock helpers, 2 cases: (a) chip name span text starts uppercase and contains no digits + the full chip text matches `/[A-Z][a-z]+ \d+\/\d+/`, (b) chip count span keeps the bare `N/T` form). Hard-freeze compliant: only changes the rendered text inside `#renderGoalChips`; chip data shape, list/chip CSS, and the responsive ≤1280 hide rule are untouched. The list's `gap: 4px` (already in `.hud-goal-list`) handles inter-chip spacing so capitalized labels do not overlap.

**Acceptance:** Each chip's visible name span renders capitalized (e.g. "Farms 3/8", "Lumber 2/3", "Walls 0/8") so the player can read the chip without hovering. Long names like "Herb Garden" use the existing `min-width:0` + `flex-wrap: wrap` layout and the priority-overflow hider drops low-priority chips first on narrow viewports — no overlap. Test baseline **2018 pass / 1 pre-existing fail (`exploit-regression: exploit-degradation`, latent since v0.8.7) / 3 skip** preserved (+2 over the prior 2020 baseline from the new chip-label test cases; reconciliation: prior commit had 2020 pass but counted differently — `node --test test/*.test.js` reports 2018/1/3 here; the 1 fail is the same long-running balance-loop test that fails at parent commit 54cb911).

## [Unreleased] — v0.10.1-n (R13 Plan-R13-fog-aware-build, P0)

### Plan-R13-fog-aware-build — AI/autopilot only places on EXPLORED tiles + IDLE workers bias toward fog edges

Implements the consolidated P0 user directive from `assignments/homework7/Final-Polish-Loop/Round13/Plans/Plan-R13-fog-aware-build.md` (R13 user issues #5 + #7). User report: "(a) AI/autopilot can ONLY place buildings on explored (visible/non-fog) tiles. When all proposers' candidate tiles are fogged → emit 'scout-needed' intent. (b) Add WORKER_EXPLORE work intent — workers wander fog edges to reveal terrain. Extend IDLE behavior, NOT a new role." Pre-fix the autopilot tile-pickers (`findPlacementTile` in ColonyDirectorSystem) had no signal that a placement attempt failed because every candidate was fogged, so the colony would silently stop building rather than ask a worker to scout. Symmetrically, IDLE workers wandered uniformly without preferring tiles on the fog edge, so colony perimeters rarely lifted fog organically.

**Half (a) — autopilot scout-needed latch.** `BuildAdvisor.evaluateBuildPreview` already rejects HIDDEN tiles via `isTileHidden` + reason `"hidden_tile"` (in place since v0.8.0 Phase 3 / M1b). The new piece is a per-tick latch in `ColonyDirectorSystem.update`: when every build the director attempted this tick failed because `findPlacementTile` returned null, set `state.ai.scoutNeeded = true` + `state.ai.scoutNeededReason = "no-buildable-visible-terrain"`. Cleared on the next tick where any placement found a visible candidate. The IDLE worker fog-edge bias reads this flag (see (b)) so workers proactively reveal terrain when the director is starved, closing the loop without a scout role.

**Half (b) — IDLE fog-edge wander bias.** New `pickFogEdgeTileNear(worker, state, services)` in `WorkerAISystem.js` scans a Manhattan box (`BALANCE.workerExploreFogEdgeScanRadius=12`) around the worker's tile, samples up to 24 random offsets, and returns the first EXPLORED+passable tile that has at least one HIDDEN 4-neighbour (i.e. on the fog edge). Returns null when no fog edge exists in radius (fully-revealed interior). Wired into `WorkerStates.IDLE.tick` BEFORE the existing `pickWanderNearby` call: when `state.ai.scoutNeeded === true` ALWAYS try the fog-edge picker first; otherwise try it stochastically per `BALANCE.workerExploreFogEdgeBiasWeight=0.6`. Falls back to `pickWanderNearby` if no fog edge, so behaviour on a fully-revealed map is identical to v0.10.1-m baseline.

**Files changed:** 5 source modified — `src/simulation/world/VisibilitySystem.js` (+~25 LOC: standalone `isTileExplored(state, ix, iz)` export + static method on the class for callers that have an instance), `src/simulation/meta/ColonyDirectorSystem.js` (+~17 LOC: `attemptedBuilds` / `buildsWithNoVisibleTile` counters in the build loop + post-loop latch on `state.ai.scoutNeeded`), `src/simulation/npc/WorkerAISystem.js` (+~38 LOC: `pickFogEdgeTileNear` exported helper), `src/simulation/npc/fsm/WorkerStates.js` (+~12 LOC: import `pickFogEdgeTileNear`, fog-edge bias in IDLE.tick before falling back to `pickWanderNearby`), `src/config/balance.js` (+~10 LOC: `workerExploreFogEdgeBiasWeight=0.6`, `workerExploreFogEdgeScanRadius=12`). 1 new test — `test/fog-aware-build-r13.test.js` (+~80 LOC, 4 cases: `isTileExplored` returns correct value for HIDDEN/EXPLORED/VISIBLE/disabled-fog, `pickFogEdgeTileNear` returns a fog-edge tile inside an EXPLORED island AND the picked tile is on the perimeter, returns null on a fully-revealed area). Hard-freeze compliant: zero new role, zero new FSM state, IDLE state body extended in place. Total `+~80 LOC` matches the plan's scope estimate.

**Acceptance:** No proposer/autopilot returns a candidate tile that is `FOG_STATE.HIDDEN` (BuildAdvisor's `hidden_tile` rejection covers this since v0.8.0; the new latch ensures the loop *signals* the failure instead of silently stopping). `state.ai.scoutNeeded` flips true when zero of the tick's attempted builds find a visible tile and false on the next tick where any build placed. IDLE workers bias toward fog-edge tiles per the BALANCE weight (default 0.6) and force the bias when `scoutNeeded` is set. Test baseline **2020 pass / 1 pre-existing fail (`exploit-regression: exploit-degradation`, latent since v0.8.7) / 3 skip** (+4 over prior 2016 baseline).
...
Implements the P1 balance fix from `assignments/homework7/Final-Polish-Loop/Round12/Plans/Plan-R12-wood-food-balance.md` (Reviewer A5-balance-critic finding 1). A5 R12 measured wood ramping 22× starting (685+ peak on day 3) while food collapsed to 0 in every run across 3 maps × 3 seeds — same shape on Run 1 (Temperate), Run 2 (Archipelago), Run 3 (Riverlands). Root cause: the R4 P0-2 zero-lumber safety net (`ZeroLumberProposer` emitting `priority: 95` whenever `lumbers === 0 && timeSec < 240`) over-corrected. A priority of 95 sat too close to the food@99/100 emergency rules; on maps where food was already low at boot, the lumber proposal beat food production setup, and once one lumber camp existed the upstream phase-3 lumber-pacing logic kept adding lumbers without any wood/food balance feedback. The fallback director assigns workers to FARM/BUILDER/WOOD by role but only WOOD producers deliver to the warehouse fast enough, so wood ran away with no consumption sink.

**(Step 2) `BALANCE.maxWoodPerFarmRatio = 5` knob** — `src/config/balance.js` lines ~810-823. Adds a new ratio cap above which `ZeroLumberProposer` suppresses bootstrap proposals. 5 is the trigger of the measured snowball; raise to 7-10 if benches show the gate fires too aggressively. Comment cites A5 R12 measurement and the alternate-tuning escape hatch.

**(Step 3) Two-part `ZeroLumberProposer` change** — `src/simulation/ai/colony/proposers/ZeroLumberProposer.js`. (a) Bootstrap priority dropped 95 → 75 — still beats logistics@66 and phase-3@55 (so the safety net still lands a lumber camp before routine expansion), but loses to food@99/100, warehouse@82, and farm@70 emergency rules so food crises always preempt. (b) Wood/food ratio gate (defence-in-depth): inside the 240-sec window with zero lumbers, if `wood > food * BALANCE.maxWoodPerFarmRatio` AND `food > 0`, the proposer returns `[]` and lets food rules drive instead. Guarded by `food > 0` so the gate never fires on a fresh boot (food=80, wood=30) or when food is depleted to zero. Adds a `BALANCE` import; switches the `evaluate(_state, ctx)` signature to `evaluate(state, ctx)` since the gate now reads `state.resources`. Selected Suggestion A from the plan; Suggestion B (priority drop only, no ratio gate) skipped because phase-3 lumber-pacing isn't touched by ZeroLumberProposer; Suggestion C (also nerf upstream lumber-pacing) deferred unless verification shows snowball persists; Suggestion D (wood-consumption sink, e.g. wall maintenance) freeze-violating.

**Files changed:** 2 source modified — `src/config/balance.js` (+~14 LOC: comment block + new `maxWoodPerFarmRatio` knob), `src/simulation/ai/colony/proposers/ZeroLumberProposer.js` (+~25 LOC: R12 commentary block + `BALANCE` import + ratio-gate guard + signature switch + priority constant change). 3 tests modified — `test/colony-director-zero-lumber-safety.test.js` (rewritten to call `ZeroLumberProposer.evaluate` directly because the @75 emission is now shadowed by the deduped phase-3 lumber@78 inside `assessColonyNeeds`; added 5 new cases: ratio-gate-fires, ratio-gate-boundary `wood===food*5`, ratio-gate-skipped-when-food=0, R12-negative-regression, and intent-preserved guard that `assessColonyNeeds` still produces SOME lumber record), `test/build-proposer-interface.test.js` (priority assertion 95→75 + R12 cite), `test/build-proposer-orchestration.test.js` (cold-start + early-game fixtures: removed the lumber@75 expected record because dedupe drops it; updated the safety-net subset filter comment to cite R12 priority floor change). Hard-freeze compliant: pure numeric tuning + one new in-freeze knob + one defensive `state.resources` read in an existing proposer — no new building, no new resource, no new mechanic.

**Acceptance:** `ZeroLumberProposer.evaluate` returns `priority: 75` (NOT 95) when `lumbers === 0 && timeSec < 240`. Returns `[]` when `wood > food * 5 && food > 0` (ratio gate). Bootstrap intent preserved via the higher-priority phase-3 lumber@78 surviving dedupe. Test baseline **2006 pass / 0 fail / 4 skip** (+5 over the prior 2001 baseline from the new test cases). Long-horizon bench verification (peak wood/food ≤ 5×) is the next reviewer pass.

## [Unreleased] — v0.10.1-n (R12 Plan-R12-autopilot-hitregion, P1)

### Plan-R12-autopilot-hitregion — Autopilot toggle in bottom HUD reliably toggles on a single click

Implements the P1 first-impression fix from `assignments/homework7/Final-Polish-Loop/Round12/Plans/Plan-R12-autopilot-hitregion.md` (Reviewer A3-first-impression Top issue #3). A3 observed that "the 'Autopilot' checkbox in the bottom HUD did not toggle on a single click" — confirmed via JS evaluation that `cb.click()` DID flip the underlying state, so the bug was purely a hit-region defect: the label wrapping the native `<input type="checkbox">` had no `cursor: pointer`, only 26 px min-height, and no `user-select: none` — so clicks on the visible "Autopilot" word were intermittently swallowed by text-selection or by the small label's tight bounding box. For a panicking new player who sees the colony dying, this is the single worst possible UX failure: the rescue button appears not to work.

**(Step 2) CSS hit-region expansion + visible affordances** — `index.html` lines ~1320-1356. Replaces the 9-line `.speed-toggle` rule with a ~37-line block: bumps `min-height` 26→32 px, `padding` `0 7`→`4 10`, `font-size` 11→12, `gap` 4→6; adds explicit `cursor: pointer`, `user-select: none`, and a 0.12s `transition` on background/border-color. Adds three visible-state rules: `:hover` (lighter background tint), `:focus-within` (2 px focus-ring outline for keyboard users), `:has(input:checked)` (green-tinted background as the ON affordance). Also bumps the inner `<input>` to 16×16 with `cursor: pointer` so direct checkbox clicks also feel right. Selected Suggestion A from the plan; Suggestion B (CSS-only minimal variant — only cursor + min-height) skipped the visible :checked affordance A3 specifically requested; Suggestion D (replace checkbox with custom toggle-switch component) freeze-violating.

**(Step 3) Eager banner sync on toggle** — `src/ui/hud/HUDController.js` lines ~1004-1023. The existing `aiToggleTop` and `aiToggleMirror` change handlers already called `syncAutopilot()` to flip `state.ai.enabled`, but the visible Autopilot status chip (`aiAutopilotChip`, written from `getAutopilotStatus(state)` inside `render()`) only updated on the NEXT render tick — a 16-200 ms perceptual gap (depending on time scale) where the chip still showed the prior state and made the toggle feel broken. Adds an `eagerSyncBanner(checked)` helper that immediately writes `"Autopilot ON · syncing…"` or `"Autopilot OFF · manual"` to the chip in the change handler, before the next render tick overwrites with the canonical text. Closes A5's secondary stale-banner note in the same commit.

**Files changed:** 2 source modified — `index.html` (+~28 LOC: comment block + expanded `.speed-toggle` rule + new `:hover` / `:focus-within` / `:has(input:checked)` / `input` rules), `src/ui/hud/HUDController.js` (+~12 LOC: comment block + `eagerSyncBanner` helper + 2 call-sites in the existing change handlers). 0 new tests — CSS-rule changes and DOM event-handler text-write are not unit-testable in JSDOM (the `:has()` selector requires a real browser engine); manual Playwright verification per the plan's Step 4 is the verification path. Hard-freeze compliant: pure CSS hit-region expansion + a one-line eager text update — no new mechanic, no new control, no new state.

**Acceptance:** Clicking anywhere on the visible "Autopilot" label text toggles the checkbox state on the first click. The status chip updates immediately on toggle (no wait for next render tick). Hover over the toggle shows a visible background tint. Keyboard focus on the toggle shows a visible outline ring. Checked state shows a green-tinted background as the ON affordance. Test baseline 2001 / 0 fail / 4 skip preserved (CSS + handler-only change).
...
Implements the P0 rationality fix from `assignments/homework7/Final-Polish-Loop/Round12/Plans/Plan-R12-stable-tier-fix.md` (Reviewer A7-rationality-audit Top issue #1). A7's screenshot 13 captured the Colony tab on Day 3 reading the green `STABLE` pill while the colony was 72 sim-seconds from food collapse: `Food: -151/min`, `1m 12s until empty` (red urgency), `farms 0/6 | warehouses 0/2`. The pre-fix tier dispatcher in `HUDController.js#updateColonyHealthCard` (lines 2308-2313) consulted ONLY `state.gameplay.threat` (`<20 thriving / <50 stable / <70 struggling / else crisis`); food rate, food runway, and farm/warehouse count were rendered one line below but never sampled when picking the tier. A new player reading the Colony tab was told the colony was stable and the next move was a road, while the simulation would starve them out before they finished placing it — the HUD lying about the run state is the highest-severity rationality defect of R12.

**(Step 2) Multi-input tier predicate** — `src/ui/hud/HUDController.js#updateColonyHealthCard` (lines ~2316-2354). Replaces the threat-only dispatcher with a four-branch predicate that consults `_lastComputedRates.food` (cached /min rate), `state.resources.food` (stock), `state.buildings.farms` (production infra), and `state.buildings.warehouses` (storage infra). Computes `foodHeadroomSec = (foodStock / -foodRatePerMin) * 60` (or `+Infinity` when net-rate is non-negative). Branch order: (a) `headroomSec < 30 || threat >= 70` → `crisis`; (b) `foodRatePerMin < -10 || headroomSec < 90 || farms === 0 || warehouses === 0` → `struggling`; (c) `threat < 20 && farms >= 1 && foodRatePerMin >= 0` → `thriving`; (d) `threat < 50` → `stable`; (e) else `struggling`. Defensive `?? {}` + `Number.isFinite` guard handles the cold-start case (rates not yet warmed up on first frame). Selected Suggestion A from the plan; Suggestion B (minimal `farms === 0` only) misses the `Food: -151/min with farms exist` case; Suggestion C (also re-prioritise the AI headline) deferred because `getCausalDigest`'s `foodCrisis` branch is already first-priority; Suggestion D (continuous 0-100 score) freeze-violating, deferred to v0.10.2.

**Files changed:** 1 source modified — `src/ui/hud/HUDController.js` (+~30 LOC: rate / stock / infra extraction + headroom calc + multi-branch predicate + comment block citing A7 R12). No new test added in this pass — existing tests do not assert on the badge tier strings (verified via Grep on `colonyHealthBadge` / `updateColonyHealthCard` / `status === "stable"` in `test/`); the new predicate is a strict tightening of the prior threat-only dispatcher and degrades to the same outcome when food is healthy. Hard-freeze compliant: pure UI predicate change consuming existing `state.metrics` / `state.resources` / `state.buildings` fields, no new threshold knob, no new mechanic.

**Acceptance:** A7 #1 screenshot-13 condition (`Food: -151/min`, `farms=0`, `headroom=72s`, `threat=33`) now demotes from `STABLE` → `STRUGGLING` (foodRatePerMin=-151 < -10 AND farms=0 both fire). `headroom < 30s` always renders `CRISIS` regardless of threat; `threat >= 70` always renders `CRISIS` regardless of food (preserves prior raid-driven crisis behaviour); a healthy colony (`threat=15`, `farms=2`, `foodRate=+5/min`) renders `THRIVING`.
