...
single-resource starvation: with food=0 and other three resources at 1.0,
the composite is ~0.682 (=2^(3/4)-1), strictly less than the arithmetic
mean (0.75). Tests both the helper and the end-to-end ``self_score`` path.
"""
...
class TestGeometricCollapse:
    def test_food_zero_with_others_one_drops_below_arithmetic(self) -> None:
        composite = crafter_geometric_mean([0.0, 1.0, 1.0, 1.0])
        arithmetic = sum([0.0, 1.0, 1.0, 1.0]) / 4
        # 2^(3/4) - 1 = 0.6817928...
        assert composite == pytest.approx(2 ** 0.75 - 1, abs=1e-4)
        # Geometric form punishes single-axis failure: well below arithmetic.
        assert composite < arithmetic
        assert composite < 0.69
    ...
    def test_two_zeros_collapses_further(self) -> None:
        ...
        composite_one = crafter_geometric_mean([0.0, 1.0, 1.0, 1.0])
        composite_two = crafter_geometric_mean([0.0, 0.0, 1.0, 1.0])
    ...
    def test_self_score_food_zero_collapses_composite(self) -> None:
        ...
        # workers=8; food=0 → food_suf=0; wood/stone/herbs all at full → 1.0
        samples = [
            {
                "t": 0.0,
                "food": 0.0,
        ...
        # Composite collapses well below arithmetic-mean expectation of 0.75.
        assert result["rae_composite"] < result["rae_sufficiency"] + 1e-6 or result["rae_sufficiency"] == 0.0
        assert result["rae_composite"] < 0.70  # JS port verified value ~0.6818
