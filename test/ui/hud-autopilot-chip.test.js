...
test("HUDController renders Autopilot OFF chip as player-controlled", () => {
  withHudDom((nodes) => {
    ...
    assert.equal(nodes.aiAutopilotChip.textContent, "Autopilot OFF - you are in control");
...
test("HUDController renders Autopilot ON chip with next policy countdown", () => {
  withHudDom((nodes) => {
    ...
    assert.match(nodes.aiAutopilotChip.textContent, /^Autopilot ON - next tick in \d+\.\ds$/);
