...
input_contract:
  ...
  - repo_root: c:/Users/user/CodesOther/Project Utopia
...
4. **必须给至少 2 个 Suggestions**。单一方案会让 orchestrator 在 plan conflict
   仲裁时没得选。
5. **freeze 边界按轮决定**。每一轮 Runtime Context 必须显式声明 `freeze_policy`
   字段：`active`（HW06 默认收紧 —— 不加新 tile/building/tool/mood/score/audio
   asset/relationship mechanic，只做 polish / fix / UX / perf）或 `lifted`
   （允许深层 economy/logistics/AI/director 重写 + 按需引入新 mechanic 以提升
   游戏完成度）。Runtime Context 没有显式声明则**默认 active**。违反所声明
   policy 的 plan 被 orchestrator 拒收。
