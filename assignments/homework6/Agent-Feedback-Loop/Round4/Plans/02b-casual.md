...
- Attempted Playwright MCP reproduction on 2026-04-23, but `mcp__playwright__.browser_tabs` / `browser_snapshot` failed before a live snapshot with: `Browser is already in use for C:\Users\user\AppData\Local\ms-playwright\mcp-chrome-7cb89f3, use --isolated to run multiple instances of the same browser`.
