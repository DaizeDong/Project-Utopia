...
- **Round-2 simplification (2026-05-10)**: `POLICY_INTENT_TO_STATE` 13→6 (dropped 7 aliases: idle/seek_rest/guard_engage/gather/haul/seek_construct/construct), `ANIMAL_SPECIES` mapping fully deleted (binary `ANIMAL_KIND` is canonical), `LEGACY_GROUP_IDS` + VISITORS contract deleted, `WarehouseQueueSystem` + `VisibilitySystem` dropped from `SYSTEM_ORDER` (no Python impl ever existed), `BUILD_COST["erase"]` deleted (erase uses inline `{"wood": 1.0}`). 617 tests pass. All 3 Python tier hashes (`be19781c / f7a099a4 / 43cb6aec`) unchanged.
